...
import DatabaseComposition from '../../assets/images/diagrams/2-database-composition.svg'
...
const CoreConceptsPage = (): React$Element<React$FragmentType> => {
    return (
        ...
        />
            ...
            {/*<ol>*/}
            {/*    The admin layer:*/}
            {/*    <li>Jayne: account and database admin, 2nd-step compilation</li>*/}
            ...
            {/*</ol>*/}
            <h3>Databases</h3>
            Estate hosts cloud backends in always-on, <em>internet-accessible machines</em> called <b>Databases</b> that
            ...
            <h3>Database Composition</h3>
            Each database is composed of two layers: <b>the database</b> and the <b>Database Kernel.</b><p/>
            <img src={DatabaseComposition} alt={"Database Composition"} className={'diagram-right'}
                 style={{width: '350px', height: '400px'}}/>
            <h4 style={{marginTop: '30px'}}>the database</h4>
            The services and objects you author live in <em>the database</em> at runtime. the database is an <em>Internet-connected
            ...
            <h4>Database Kernel</h4>
            The Database Kernel is an object-oriented <em>cloud operating system</em> that manages and provides internet
            access to the database at runtime.
            <p/>
            <p/>
            ...
            <p/>
            The Database Kernel is programmable by Database developers via the <code>estate</code> TypeScript library. This
            library is created when the <code>estate database init</code> command is run.
            <h3>Hosting in the database</h3>
            Hosting a class in the database is easy. Simply extend one of three abstract base
            classes: <b>Service</b>, <b>Data</b>, or <b>Message</b> (not shown, more on this later) found in
            the <code>estate</code> library. Which abstract class you choose determines how instances of that class
            will behave in the database and how clients use them at runtime.
            ...
            <CodeBlock code={`const myService = database.getService(MyService,"my primary key")`} language={"javascript"}/>
            ...
            identifies the service instance living the database.
            <p/>Service instances are created automatically the first time a method is called on them. Additionally,
            they live until you explicitly delete them (see the Database Runtime API documentation for more details).
            ...
            Space. At runtime, inside the service method code running in the database, <code>this</code> is the service
            object instance.
            <h5>Units-of-work</h5>
            ...
            <CodeBlock code={`let myData = await database.getDataAsync(MyData,"a primary key");`}
                       language={'javascript'}/>
            ...
            <h5>Retrieving existing data objects from a service in the database</h5>
            ...
            The arguments are the same as the clients-side example above. This loads the object into the database memory
            ...
            directly from the client using the <code>database.saveDataAsync</code> function. Though the Data
            ...
            first subscribing to updates with <code>database.subscribeUpdatesAsync</code> and then attaching a handler
            function with <code>database.addUpdateListener</code>. The example application (link below) has a
            ...
            “default” (because that’s what the client specified in the call to <code>database.getService(MyService,
                "default")</code>).
